...
class BackgroundGridRenderSystem extends RenderSystem {
  ...
  override function render(renderer:Renderer, forge:Forge) {
    ...
    renderer.drawRect(-50 * 32, -50 * 32, 100 * 32, 100 * 32, 0, {color: 0xFF444444});

    for (x in -50...50) {
      for (y in -50...50) {
        renderer.drawRect((32 * x) + 1, (32 * y) + 1, 30, 30, 0, {color: 0xFF777777});
      }
    }

    renderer.drawRect(0, 0, 50, 50, 0, {color: 0xFFFFFF00});
