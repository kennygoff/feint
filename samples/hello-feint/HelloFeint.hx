...
class HelloFeint extends Application {
	...
	override function update(elapsed:Float) {
		...
		if (window.inputManager.keyboard.keys[KeyCode.Enter] == JustPressed) {
			rectColor = 0xFF000000 | (Math.floor(Math.random() * 0xFF) << 16) | (Math.floor(Math.random() * 0xFF) << 8) | Math.floor(Math.random() * 0xFF);
		}
	}
	...
	override function render(renderer:Renderer) {
		...
		// Render rectangle
		renderer.drawRect(Math.floor(window.width / 2 - 25), Math.floor(window.height / 2 - 25), 50, 50, {color: rectColor});
